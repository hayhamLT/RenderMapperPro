...
- **Asset grouping → previz auto‑export** — for shows on a structured naming convention (e.g. `PRJ001_D01_S01_A017_CENTER_ANIM_V003`), the watch folder groups dropped clips by **setup + asset**, maps each **screen** code to its material, keeps the **newest version**, and exports **one multi‑screen previz render per asset** — drop 10 clips, get 5 assets. The parser (regex), content‑type filter, output‑name tokens, screen→material overrides and per‑setup scene routing are all configurable in *Properties → Watch & Auto‑render → Asset grouping*. A newer version updates that asset's job in place rather than piling up.
